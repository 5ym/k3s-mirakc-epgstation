...
/** PAT の table_id。ここだけのために `service-filter.ts` を引っ張らない */
const TABLE_PAT_ID = 0x00;

/**
 * PAT と PMT のパケットだけを拾って持っておく。**次に開くときの呼び水にする。**
 *
 * ffmpeg は入口で PAT と PMT を待ってから中身を読み始める。選局のやり直しには
 * 0.66秒かかるが、**その間 ffmpeg には何も渡せていない** — 呼び水を先に流して
 * おけば、電波が来る頃には入口の仕事が終わっている。実測で 0.18 秒縮んだ
 * ([stream.md](../../../docs/stream.md) §4)。
 *
 * **PSI は物理チャンネルごと**なので、覚えておくのもチャンネル単位でよい。
 * 中身が古くなっていても、本物が 0.1 秒ごとに来て上書きするだけ。
 *
 * 揃ったら止める。PSI は流れ続けるが、拾い直す値打ちは無い。
 */
export class PsiTap {
    private readonly stream = new PacketStream();
    private readonly pat = new SectionAssembler(0);
    private readonly kept: Uint8Array[] = [];
    /** PAT が名乗った PMT の PID。読むまでは null */
    private wanted: Set<number> | null = null;
    private readonly seen = new Set<number>();

    /** 揃ったか。**揃ったあとは何もしない** */
    get full(): boolean {
        return this.wanted !== null && this.seen.size >= this.wanted.size;
    }

    feed(chunk: Uint8Array): void {
        if (this.full) return;
        for (const packet of this.stream.feed(chunk)) {
            const pid = ((packet[1] & 0x1f) << 8) | packet[2];
            if (pid === 0) {
                this.kept.push(packet.slice());
                for (const section of this.pat.feed(packet)) this.readPat(section);
            } else if (this.wanted?.has(pid)) {
                this.kept.push(packet.slice());
                this.seen.add(pid);
            }
            if (this.full) return;
    ...
    private readPat(section: Uint8Array): void {
        if (section[0] !== TABLE_PAT_ID) return;
        const found = new Set<number>();
        for (let at = 8; at + 4 <= section.length - 4; at += 4) {
            // program_number 0 は NIT の在り処で、局ではない
            if (((section[at] << 8) | section[at + 1]) === 0) continue;
            found.add(((section[at + 2] & 0x1f) << 8) | section[at + 3]);
        }
        if (found.size > 0) this.wanted = found;
    }

    /** 呼び水。**揃っていなければ null** — 半端なものを流しても意味が無い */
    primer(): Uint8Array | null {
        if (!this.full) return null;
        const out = new Uint8Array(this.kept.length * PACKET);
        for (const [index, packet] of this.kept.entries()) out.set(packet, index * PACKET);
        return out;
    }
}
