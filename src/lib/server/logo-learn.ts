...
export function missing(): Target[] {
    return pending(
        queryAll<Target>(`
            SELECT id, service_id, name, type, channel
            FROM services
...
export function stats(): { have: number; total: number } {
    const services = queryAll<{ id: number }>(
        `SELECT id FROM services WHERE ${CURRENT_SERVICES} AND service_type = 1`,
    );
