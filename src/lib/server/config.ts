...
export const config = {
    ...
    jlsLogoSamples: 600,
    /**
     * 「ずっと同じ縁がある」と認める強さ。logoframe の既定は 40。
     *
     * **上げてある。** 自動探索が見ているのは画面の右上だけで、しかも手がかりは
     * これ1つなので、弱い縁 (常時出ている枠や下地) まで拾うとロゴでないものを
     * 覚える。実機の TOKYO MX は 48×158 の縦長の帯を覚えていて、中身は横縞の
     * 雑音、濃さも 268/1000 止まりだった。それでも「毎フレーム合致する」ので
     * 作り直されず、CM判定は毎回 100% になって捨てられ続けていた。
     *
     * 見つけられない局が増える方向だが、そちらは画面に出て手で教えられる。
     * 黙って雑音を覚え続けるより直しようがある
     */
    jlsLogoEdgeThreshold: 60,
    /**
     * 覚えているロゴの合致率がこれ(%)を下回ったら覚え直す。logoframe の既定は 10。
     *
     * **上げてある。** 10 では、上のような雑音 (どのコマにも合致する) が一度
     * 入り込むと永久に作り直されない。本編でロゴが出ているのは番組の8割ほどな
     * ので (CM中は消える)、50 なら普通のロゴは作り直しにならない
     */
    jlsLogoMatch: 50,
