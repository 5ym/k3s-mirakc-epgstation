 ...
 * 決まる。字幕は時刻ではなく**待たせる量**で合わせている (`Notice` の `tuned`
 * と `lead`) ので、この値を使うのはデータ放送 (第3段階) からになる。
 */
...
export type Notice =
    | {
           ...
           * 焼くのに時間がかかる)。どれだけ先かは焼き方・コマ数・局で変わり、
           * どれもサーバ側にしか無いので、ここで決めて渡す
           * (`server/live.ts` の `captionLead`)。**局のぶんは選局した時点では
           * まだ分からない** ので、あとから `lead` で言い直す
           */
          lead: number;
