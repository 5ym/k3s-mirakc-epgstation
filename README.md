...
1. ライブラリの種類を **番組(Shows)** にして `/library` を追加する。
2. **メタデータ保存形式に「Nfo」を有効**にし、**インターネットのメタデータ取得は無効**に
   する。日本の放送番組は TheTVDB / TMDB にほぼ載っておらず、有効なままだと denpa が
   書いた `.nfo` が空の検索結果で上書きされます。
3. 録画を Jellyfin から消したい場合は、ユーザー設定で **「メディアの削除を許可」**を
   有効にする(ライブラリは読み書きでマウントしてあります)。
