...
**`devices:` だけです。** コンテナに `/dev/bus` と `/dev/dvb` を見せるところで、
ここだけは Docker にしか書けません。**あとは全部、画面の「チューナー」から**
変えられます — 本数・デバイス・受けられる種別・LNB・「1本だけ止める」まで。

そもそも**書かなくても動きます。** 定義が無ければエージェントが `/dev/dvb/*` を
開いて、地上波か衛星かまで自分で判別します。
