...
| `/live` | **いま流れているものを見る** ([stream.md](stream.md))。左に映像、右にチャンネル。**前回見ていた局から開く** (覚えていなければリモコン番号のいちばん若い局)。局の並びは番組表と同じなので、あちらで見つけた局をここでも同じ位置で探せる。**操作列は自前** (備え付けは使わない) で、止めた所から見られる・音声を選べる・切り替えの間は前の絵を貼る。**第1段階なので映像と音声だけ** — 字幕とデータ放送はまだ出ない |
