...
| `/tuners` | チャンネルスキャン (途中で中断できる)、**mirakc の入れ直し** (局を増やせるのはこれだけ)、チューナーの空き、取れているチャンネル (mirakc 側の局と番組表の集まり具合つき)、mirakc とカードリーダーと局ロゴの状態 (**局ロゴを今すぐ取りに行く**: 地上波だけをチューナー2つで。進み具合が出る) |
