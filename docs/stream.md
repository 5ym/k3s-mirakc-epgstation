...
> **同じ落ち方が、先出しとは無関係にも起きる。** 局が3つ相乗りしている TS
> （実機の tvk / T15。tvk1・2・3 + ワンセグ + データで、局ごとに14本以上の
> ストリームがある）では、`-probesize 400000` が足りず `-map 0:p:24632:v:0` を
> 解決できないまま ffmpeg が降りる。**元からある不具合で、まだ直っていません。**
> どこまで上げれば足りるか（`-analyzeduration` で時間の頭打ちを付けられるか）は
> 未測定 — 上げると全チャンネルの立ち上がりが遅くなるので、数字を見てから決める。
>
> ただし**黙って黒くなることはなくなった**。ffmpeg が降りたらその場で
