...
> **第1段階 (映像音声) だけ入っています。** 字幕・データ放送・低遅延の追い込みは
> まだ設計だけです。どこまで入っているかは [§4](#4-実装の順序) を見てください。
> [roadmap.md](roadmap.md) の「ライブ視聴」に対応します。
...
| 相乗り | 同じチャンネルを2人で見ても、チューナーも ffmpeg も1つ |
...
**`tsreadex` はまだ挟んでいない。** デュアルモノの無劣化分離と PID 固定はあちらの
仕事だが、イメージに入れるところからになるので第1段階では省いた。**音声が
デュアルモノの番組で崩れる可能性がある** — 字幕（第2段階）で ffmpeg をもう1本
足すときに一緒に入れる。
