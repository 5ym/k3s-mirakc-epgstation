...
削り方は2つある。どちらも**待ち時間を重ねる**だけで、短くはしていない。

- **画面が繋いでくる前に焼きはじめる**（`server/live.ts` の `warm`）。開く局は
  ページを組む時点で決まっているので、上の 160ms を ffmpeg の 530ms に重ねられる
- **PSI を先に流しておく**（`ts/psi.ts` の `PsiTap`）。ffmpeg は入口で PAT と PMT を
  待つが、選局のやり直しの 0.66 秒の間、あちらには何も渡せていない。前に開いたときの
  PAT/PMT を取っておいて先に流すと、電波が来る頃には入口の仕事が終わっている

  | | 本物が流れてから絵が出るまで |
  | --- | --- |
  | 先出しなし | 中央値 888ms（603〜945、ばらつき 342ms） |
  | **PSI を 180ms 先出し** | 中央値 **707ms**（665〜742、ばらつき **77ms**） |

  先出しした 180ms がほぼそのまま返る。**ばらつきも縮む** — 残るのが I フレーム待ち
  だけになるため。古い PSI でも害は無い（本物が 0.1 秒ごとに来て上書きする）
...
- **字幕にいつ出すかは添えない。頭の PTS も使わない。** 受け側は**届いた時点の
