...
**相乗りの単位は局とコマ数。** 1本の物理チャンネルに複数の局が乗っていて、
ffmpeg には局を名指しさせている（下記）ので、出てくる絵が局ごとに違う。
コマ数も同じで、国内アニメを見ている人と実写を見ている人では違う。
...
| 欲しかったもの | 結論 |
| --- | --- |
| デュアルモノ分離 | ffmpeg で足りる。**録画と同じ見分け方**（番組表の `audio_type`）で、主音声を両耳へ配る。録画は左右を2トラックに分けるが、ライブは器が1つなので主音声を採る |
