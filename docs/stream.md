...
channel:
  ...
  0x11  audio media
  0x20  字幕の絵     [4:何ms前か(符号付)][2:x][2:y][2:w][2:h][PNG...]
  0x21  字幕を消す   [4:何ms前か(符号付)]
...
- **字幕の時刻は「いま焼いている絵より何ms前か」で送る。** 絶対の時刻では合わない —
  mp4 は必ず 0 から始まり、その 0 がどの放送時刻にあたるかは多重化器の都合で決まる
  (音声のほうが先に溜まっているとそちらに合う)。実機で 2.4 秒ずれ、字幕がそのぶん遅れた。
  受け側は自分の持っている端から戻すだけでよい
