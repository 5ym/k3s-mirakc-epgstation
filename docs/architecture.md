...
### 字幕は ASS (`SUBTITLE_MODE`)

ARIB字幕は libaribcaption で解いて、**文字のまま ASS で持ちます**。位置も色もルビも
ASS のタグで残り、Matroska が元から想定している形なので VLC も Kodi も Infuse も
素直に出します。

以前はビットマップに焼いて dvbsub で入れていました (再生側のフォントに依存しない、
という理由)。**VLC が dvbsub の入った mkv で落ちる**ので替えています。
引き換えに出す側のフォント頼みになるので、ASS に書き込むフォント名は
「イメージに入れてあるもの → 各OSの丸ゴシック」の順に並べてあります。
...
さらに)。時間のほうが惜しければ `CM_DETECTOR=silence` に戻せます。
