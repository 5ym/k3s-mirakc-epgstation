...
ARIB字幕は libaribcaption で解いて、**文字のまま (ASS) と、絵に焼いたもの (dvbsub) の
...
| | 良いところ | 困るところ |
| --- | --- | --- |
| ASS | Matroska が元から想定している形。VLC も Kodi も Infuse も素直に出す。位置・色・ルビも残る | **外字 (DRCS) を絵に置き換えられない** (libaribcaption の `replace_drcs` はビットマップ専用)。点滅や背景の箱も表せない |
| dvbsub | 放送されたとおりに出る。色は256色まで使える (4色までなのは DVD の字幕のほう) | **VLC がこれの入った mkv で落ちる** |

そこで **ASS を既定のトラック**にして VLC を通し、原寸で見たいときのために焼いたほうも
残しています。ASS に書き込むフォント名は「イメージに入れてあるもの → 各OSの丸ゴシック」の
順。`ass_single_rect` も入れています (VLC は複数の矩形を扱えず、字幕が重なって出る)。
