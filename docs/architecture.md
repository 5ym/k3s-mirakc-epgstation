...
ARIB字幕は libaribcaption で解いて、**文字のまま (ASS)・放送どおりの絵 (PGS)・
焼いたもの (dvdsub)** の3本を入れます。字幕は数十KBしかないので損がありません。
...
| PGS | **描いたままが入る** (1枚256色 + 画素ごとの透明度)。外字もそのまま絵になる | ffmpeg に符号器が無いので denpa が書いている。再生側は VLC・Kodi・Infuse・Plex いずれも読める |
| dvdsub | 外字もそのまま絵になる。古い機器でも読める | **1枚あたり4色まで**。実測230色の字幕から、縁のなめらかさと話者ごとの色分けが落ちる |
...
**ASS を既定のトラック**にして、原寸で見たいときは PGS へ切り替えます。
...
> 絵のほうは描くものがあったときだけ出るので、**本編に字幕が無い番組では絵のトラックが空**に
> なります (実機: 報道番組 ASS 196 / 焼き 101、字幕の無いアニメ ASS 52 / 焼き 0)。
